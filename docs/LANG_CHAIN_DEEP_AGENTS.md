...
agent = create_deep_agent(
    ...
    skills=["./path/to/executive-coordination/skills/"],
)
```
...
SkillsMiddleware(
    backend=backend,
    sources=["/skills/", "/path/to/executive-coordination/skills/"],
)
```

```bash
npx skills add SpillwaveSolutions/executive-coordination --skill '*' --yes
```
...
python3 path/to/second-brain-core/scripts/brain_session.py open \
  --repo "$BRAIN_REPO" \
  --bundle knowledge \
  --actor deep-agents/executive-coordination \
...
```bash
export SECOND_BRAIN_IDENTITY="deep-agents/executive-coordination"
