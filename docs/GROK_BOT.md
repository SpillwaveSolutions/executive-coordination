...
- Actor string: `grok-bot/executive-coordination`
- Claim per process with `--author grok-bot/executive-coordination` or `SECOND_BRAIN_IDENTITY=grok-bot/executive-coordination`
- Do **not** use a single shared `knowledge/.identity.json` for a fleet.
- Chat prefix (optional): `Grok Bot: Chief of Staff`
...
export SECOND_BRAIN_IDENTITY="grok-bot/executive-coordination"
```
...
**Required:** type ownership. This pack may write: Priority, Decision, Blocker, Escalation, DailyDigest, ActionItem. Refuse everything else.
