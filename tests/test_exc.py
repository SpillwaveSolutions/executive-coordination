...
SCRIPT = ROOT / "scripts" / "exc_common.py"
...
def test_write_requires_author():
    with tempfile.TemporaryDirectory() as td:
        ...
        run("init-bundle", "--bundle", str(bundle), "--title", "Test", "--catalogs", "decisions")
        r = run(
            "write",
            "--bundle",
            str(bundle),
...
def test_init_and_write():
    with tempfile.TemporaryDirectory() as td:
        bundle = Path(td) / "knowledge"
        r = run("init-bundle", "--bundle", str(bundle), "--title", "Test", "--catalogs", "decisions")
        assert r.returncode == 0, r.stdout + r.stderr
        r = run(
            "write",
            "--bundle",
        ...
        assert r.returncode == 0, r.stdout + r.stderr
        data = json.loads(r.stdout)
        assert data["author"] == "grok-bot/executive-coordination"
        assert data["event"]
...
def test_isolation_two_sessions_do_not_clobber():
    with tempfile.TemporaryDirectory() as td:
        ...
        r = run("init-bundle", "--bundle", str(knowledge), "--title", "Shared", "--catalogs", "decisions")
        assert r.returncode == 0, r.stdout + r.stderr
        git(repo, "add", ".")
        ...
        b = run_session(
            ...
            "knowledge",
            "--actor",
            "deep-agents/executive-coordination",
            "--host",
            "deep-agents",
            "--plugin",
            "executive-coordination",
        ...
        assert (Path(sa["bundle"]) / "decisions" / "session-a-note.md").exists()
        assert not (Path(sa["bundle"]) / "decisions" / "session-b-note.md").exists()
        assert (Path(sb["bundle"]) / "decisions" / "session-b-note.md").exists()
        assert not (Path(sb["bundle"]) / "decisions" / "session-a-note.md").exists()
        ...
        assert (knowledge / "decisions" / "session-a-note.md").exists()
        assert (knowledge / "decisions" / "session-b-note.md").exists()


def test_sample_pack_walks():
    root = "Northstar Priority"
